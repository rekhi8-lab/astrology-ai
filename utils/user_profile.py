from __future__ import annotations

USER_PROFILE = """User Birth Profile (authoritative):

Name: Harmanjeet Rekhi
Date of Birth: 8 January 1982
Time of Birth: 10:30 PM
Place: Jalandhar, India

Natal Chart (core placements):
- Sun: Capricorn 18° (House 4)
- Moon: Cancer 2° (House 10)
- Mercury: Aquarius 4° (House 5)
- Venus: Aquarius 7° (House 5)
- Mars: Libra 10° (House 1)
- Jupiter: Scorpio 7° (House 2)
- Saturn: Libra 21° (House 2)
- Ascendant: Virgo 20°
- Midheaven: Gemini 19°

Current Dasha Context:
Saturn Mahadasha (2013–2032)
Current Antardasha: Saturn / Mars (2025–2026 approx)

This is the user's complete chart. Use this for all interpretations."""
